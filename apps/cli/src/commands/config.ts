...
import { getControlPlaneUrl, getProviderSettingsPath, loadCliProviderSettings } from "../utils/config.js"

export async function runConfigCommand(args: string[] = []): Promise<number> {
	const json = args.includes("--json")
	const settings = await loadCliProviderSettings()
	const gateway = createKyrnoxGateway({ providerConfigs: settings.providers })
	const output = {
	...
	if (json) console.log(JSON.stringify(output, null, 2))
		...
		console.log(`Kyrnox config`)
		console.log(`Control plane: ${output.controlPlaneUrl ?? "not configured"}`)
